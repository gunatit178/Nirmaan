...
- **Cost estimation, from real usage, never a guess.** `CompletionResult.usage` (`src/lib/providers/types.ts`) now carries real input/output token counts from the Anthropic API (`AnthropicProvider` populates it from the actual response; `MockProvider` leaves it `undefined` unless a test explicitly sets one — it never fabricates a number standing in for a real call). `src/lib/costs/pricing.ts` + `estimateCost.ts` turn that into a USD estimate for models it has pricing for, and return `priced: false` (not a wrong guess) for anything it doesn't — the pricing table needs manual updates when prices change; there's no way to fetch it automatically. `dispatch.ts` logs a cost-tagged line after every dispatch (`~$0.0123 (500 in / 200 out tokens, claude-sonnet-5)`, or an honest `cost unknown` when the provider reported no usage or the model isn't priced).
